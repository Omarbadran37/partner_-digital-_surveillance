...
partner_3$old_dc<- if_else(partner_3$old_dc == 1, "yes","no")


partner_3$old_dc <- factor(partner_3$old_dc, levels=c("yes","no"))


g2 <- partner_3 %>%
  ggplot(aes(x = old_dc,fill= old_dc))+
  geom_bar()+
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
...
sofa_graph%>%
  ...
  xlab("SOFA")+
  ylab("score")+
  ggtitle("SOFA Score One - Day Three")
...
lactate_graph%>%
  ...
  xlab("lactate")+
  ylab("count")+
  ggtitle("Lactate Day One - Day Three")
...
train_fit_rf <- fit(final_rf,zero_los_train)
...
one_los_train <- one_los_train_index %>%
  select(ends_with(c("1","2")), Age, ELIX, ICU_Cat,in_partner,ICD10_Name3:ICD10_Name6)
...
train_fit_rf_1 <- fit(final_rf_1,one_los_train)
...
train_fit_rf_2 <- fit(final_rf_2,two_los_train)
...
### test script to find top VIP
# vip_test <- final_gb %>%
#   fit(data = zero_los_train) %>%
#   pull_workflow_fit() %>%
#   vip(geom = "point",num_features = 10, horizontal = F)
# 
# vip_test$data
# 
# test_vars <- slice_head(vip_test$data, n=15)
#  test_pivot <- colnames(test_vars %>%
#    pivot_wider( names_from = Variable, values_from = Importance))
#  
# test_select <- zero_los_test %>%
#   select(test_pivot, in_partner)
# 
# train_select <- zero_los_train %>%
#   select(test_pivot, in_partner)
# 
# glimpse(vip_test$data)
# 
# final_wf <- gb_workflow %>%
#   update_recipe(recipe(in_partner~., data = train_select))
# 
# final_gb<- finalize_workflow(
#   final_wf,
#   gb_best
...
train_fit_gb <- fit(final_gb,zero_los_train)
...
train_fit_gb_1 <- fit(final_gb_1,one_los_train)
...
train_fit_gb_2 <- fit(final_gb_2,two_los_train)
